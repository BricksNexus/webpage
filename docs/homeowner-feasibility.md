...
1. **`POST /api/property`** — Accepts `{ "address": "…", "metroOverride"?: "nyc" | "boston" | "generic" }`. Geocodes with **Mapbox/Google if set**, else **free U.S. Census one-line** (US), else **OSM Nominatim**, else **parse-only demo** (no OSM/Census), then:
...
# MAPBOX_ACCESS_TOKEN or GOOGLE_MAPS_GEOCODING_API_KEY (optional — /api/property uses free Census/Nominatim if unset)
