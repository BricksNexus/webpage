 ...
 *
 * Context: CODEBASE.md (lib/open-property section) | API.md (external APIs section)
 */
...
export async function fetchPropertyIntel(address, options = {}) {
  ...
  // AttomData-enriched fields — fall back to PLUTO ownerName for NYC addresses
  const plutoOwner = intel.localRecords?.nyc?.pluto?.ownerName ?? null;
  intel.ownerName = derivedSummary.ownerName ?? plutoOwner;
  const pluto = intel.localRecords?.nyc?.pluto;
  const plutoBlockLot = pluto?.block && pluto?.lot
    ? `Block ${pluto.block} / Lot ${pluto.lot}`
    : null;
  intel.blockLot = derivedSummary.blockLot ?? plutoBlockLot;
